...
- 🧰 **实用的 Skills 系统**：可从 `~/.morph`、`~/.claude`、`~/.codex` 发现并注入 `SKILL.md`，支持智能路由和显式控制（见 [../skills.md](../skills.md)）。
...
- `telegram.group_trigger_mode=talkative` 会让每条群消息都进入 addressing LLM 判定；阈值由 `telegram.talkative_addressing_confidence` 控制。
